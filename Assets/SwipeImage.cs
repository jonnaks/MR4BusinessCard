...
public class SwipeImage : MonoBehaviour
{
	...
	void Start ()
	{
		amount = GameObject.FindGameObjectsWithTag("portfolio").Length; // amount of models
		portfolio = new GameObject[amount];
		foreach(GameObject gameObj in GameObject.FindGameObjectsWithTag("portfolio"))  // iterate through model names
		{
			Debug.Log(gameObj);
			portfolio[i] = gameObj;
			//	portfolio[i].GetComponent<RectTransform>().localPosition = new Vector3 (originalPosition.x,originalPosition.y,originalPosition.z); // set to original position
			portfolio[i].SetActive(false);
//			model[i].SetActive (false);
			i++;
		}
		i = 0;
		portfolio [0].SetActive (true);
//		model[0].SetActive (true);
		Debug.Log(amount);
		for(int j = 0; j < amount; j++ ) {
			Debug.Log (j + ": " + portfolio[j]); 
		}
	}
	...
	public void Swipe ()
	{
		...
		if (Input.GetMouseButton(0)) {
			...
			if(deltaPosition.x < 0.0f){
				portfolio[i].GetComponent<RectTransform>().localPosition = new Vector3(deltaPosition.x * SlidingSpeed, 0, -6); // portfolio[i].GetComponent<RectTransform>().localPosition.x + 
				//portfolio[i].transform.Translate(Vector3.left  * 500 * Time.deltaTime);
			} else if (deltaPosition.x > 0.0f) {
				portfolio[i].GetComponent<RectTransform>().localPosition = new Vector3(deltaPosition.x * SlidingSpeed, 0, -6); // portfolio[i].GetComponent<RectTransform>().localPosition.x + 
				//portfolio[i].transform.Translate(Vector3.right * 500 * Time.deltaTime);
			}
		}
		...
		if (Input.GetMouseButtonUp (0)) {
			...
			if (currentSwipe.x < 0 && currentSwipe.y > -0.5f && currentSwipe.y < 0.5f) {
				...
				portfolio[i].GetComponent<RectTransform>().localPosition = new Vector3 (0,0,-6); // set to original position
				portfolio[i].SetActive (false);
//				model[i].SetActive (false);	
